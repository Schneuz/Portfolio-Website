...
type TerminalTypingMotionComponent = React.ComponentType<
  Omit<HTMLMotionProps<'span'>, 'ref'> & React.RefAttributes<HTMLElement>
>
...
export function AnimatedSpan({
  ...
  return (
    <motion.div
      ...
      className={`grid text-sm font-normal tracking-tight ${className}`.trim()}
      onAnimationComplete={() => {
        if (!sequence) return
...
export function TypingAnimation({
  ...
  const MotionComponent =
    motionElements[Component] as unknown as TerminalTypingMotionComponent
  ...
  return (
    <MotionComponent
      ref={elementRef}
      className={`text-sm font-normal tracking-tight ${className}`.trim()}
      {...props}
    >
      {displayedText}
    </MotionComponent>
...
export function Terminal({
  ...
  const content = (
    <div
      ref={containerRef}
      className={`z-0 h-full min-h-[160px] max-h-[260px] w-full max-w-lg rounded-[var(--radius-small)] border border-[var(--color-border)] bg-[var(--color-surface)] ${className}`.trim()}
    >
      <div className="flex flex-col gap-y-2 border-b border-[var(--color-border)] p-4">
        <div className="flex flex-row gap-x-2">
          <div className="h-2 w-2 rounded-full bg-[var(--color-accent-soft)]" />
          <div className="h-2 w-2 rounded-full bg-[var(--color-accent)]" />
          <div className="h-2 w-2 rounded-full bg-[var(--color-border-strong)]" />
        </div>
      </div>
      <pre className="p-4">
        <code className="grid gap-y-1 overflow-auto font-mono text-[13px] text-[var(--color-text-primary)]">
          {wrappedChildren}
        </code>
      </pre>
    </div>
